...
class RelatedYouTubeVideos_Widget extends WP_Widget {
   ...
   */
    ...
    $excludeList  = isset( $data['exclude'] ) ? (array) $data['exclude'] : array();
    ...
    $html .= " </li>\n";
    $html .= ' <li>' . "\n";
    $html .= '  <label for="' . $this->get_field_id( 'exclude' ) . '" style="display:inline-block;width:75px;text-align:right;;">' . __( 'Hide on:', $this->slug ) . '</label>' . "\n";
    $html .= '  <input type="text" name="' . $this->get_field_name( 'exclude' ) . '" value="' . $exclude . '" />' . "\n";
    $html .= '  <span style="display:block;padding-left:75px;">Comma separated list of post IDs</span>';
    $html .= ' </li>' . "\n";
    $html .= "</ul>\n";
...
// @todo autoplay yes/no 
   ...
   */
    ...
    // If current post ID is listed as EXLUDE, don't show the widget!
    if( isset( $data['exclude'] ) && is_array( $data['exclude'] ) && isset( $post->ID ) && in_array( (int) $post->ID, $data['exclude'] ) ) {
