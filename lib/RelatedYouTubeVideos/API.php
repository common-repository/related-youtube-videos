...
class RelatedYouTubeVideos_API {
  ...
  public function searchYouTube( $args ) {
    ...
    if( $random > $max ) {
      ...
      $target     = 'https://gdata.youtube.com/feeds/api/videos?q=' . $searchTerms . '&orderby=' . $orderBy . '&start-index=' . $start . '&max-results=' . $random . '&v=2';
      
    }
    else {
  
      $target     = 'https://gdata.youtube.com/feeds/api/videos?q=' . $searchTerms . '&orderby=' . $orderBy . '&start-index=' . $start . '&max-results=' . $max . '&v=2';
    ...
    if( $random >= $max ) {
      ...
      /**
       * @change v20170409 New Random Draw algorithm
       */

      // First {random=""} number of videos from the results
      $randomPool     = array_slice( $result, 0, $random );
    
      $dimRandomPool  = count( $randomPool );
      
      // shuffle
      shuffle( $randomPool );

      if( $max >= $dimRandomPool ) {
        $max = $dimRandomPool;
      }
      ...
      // Get {max=""} number of videos from the pool
      $randResults      = array_slice( $randomPool, 0, $max );
  ...
  public function displayResults( $results, $args = array() ) {
    ...
    // Hide on EXCLUDE list of POST IDs
    if( isset( $post->ID ) && isset( $args['exclude'] ) && is_array( $args['exclude'] ) ) {
      foreach( $args['exclude'] as $hideOnPostID ) {
        if( (int) $hideOnPostID === $post->ID ) {
          return;
        }
      }
    }
    ...
    if( !is_array( $results ) || empty( $results) ) {
      ...
      if( isset( $options['customMessage'] ) && trim( $options['customMessage'] ) !== '' ) {
        ...
        $errMsg .= htmlspecialchars( $options['customMessage'], ENT_COMPAT );
...
if( typeof showRelatedVideo !== 'function' ) {
  function showRelatedVideo( config ) {
    ...
    video += '<object data="https://www.youtube.com/embed/' +  config.videoID + '?autoplay=1&rel={$viewRelated}" width="' + config.width +  '" height="' + config.height + '">';
    video += ' <param name="movie" value="https://www.youtube.com/v/' + config.videoID + '?rel={$viewRelated}" />';
...
EOF;
      ...
      foreach( $results as $video ) {
        ...
        if( $videoID != null ) {
          ...
          $html           .= '     <img src="https://img.youtube.com/vi/' . $videoID . '/0.jpg" alt="' . $videoTitle . '" width="' . $width . '" height="' . $height . '" />' . "\n";
      ...
      foreach( $results as $video ) {
        ...
        if( $videoID != null ) {
          ...
          $html           .= '    <object data="https://www.youtube.com/embed/' . $videoID  . '?rel=' . $viewRelated . ( $autoplay === 1 ? '&autoplay=1' : '' ). '" width="' . $width . '" height="' . $height . '">' . "\n";
          $html           .= '     <param name="movie" value="https://www.youtube.com/v/' . $videoID . '?rel=' . $viewRelated . '" />' . "\n";
          $html           .= '     <param name="wmode" value="transparent" />' . "\n";
          $html           .= '     <param name="allowfullscreen" value="true" />' . "\n";
          $html           .= '     <a href="https://www.youtube.com/watch?v=' . $videoID . '"><img src="https://img.youtube.com/vi/' . $videoID . '/0.jpg" alt="' . $videoTitle . '" /><br />YouTube Video</a>' . "\n";
          $html           .= "    </object>\n";
...
*/
    ...
    /* Do not show widget on certein Post IDs */
    $excludeTMP = ( isset( $args['exclude'] ) ) ? ( is_string( $args['exclude'] )  ? (array) explode( ',', $args['exclude'] ) : (array) $args['exclude']  ) : array();

    $exclude    = array();
        
    foreach( $excludeTMP as $excludeItem ) {
      
      $excludeID  = (int) $excludeItem;
      
      if( $excludeID > 0 && !in_array( $excludeID, $exclude ) ) {
